...
NS_DEF_NARAN{
	...
	void Path::setChars(c_str path)
	{
		...
		}else if (0 != strcmp(path, mPathChars.get())){
			mPathChars = new char[strlen(path) + 1];
			strcpy(mPathChars.get(), path);
		}
	}

	more(char) Path::getName() const
	{
	...
	more(char) Path::getRawName() const
	{
		char *path = mPathChars.get();
		char *nameStart, *nameEnd;
		getNames(path, nameStart, nameEnd);
	...
	more(char) Path::getExtName() const
	{
		char *path = mPathChars.get();
		char *nameStart, *nameEnd;
		getNames(path, nameStart, nameEnd);
	...
	more(char) Path::getDirectory() const
	{
		char *path = mPathChars.get();
		char *nameStart, *nameEnd;
		getNames(path, nameStart, nameEnd);
	...
	Path Path::appendName(c_str subName) const
	{
		char *path = mPathChars.get();
