/*
 * "$Id: mxmldoc.c,v 1.1.1.1 2008/12/19 01:08:14 fyang Exp $"
 *
  ...
  for (i = 1; i < argc; i ++)
	...
	*/
	  ...
	  usage(NULL);
        ...
        xmlfile = argv[i];
	// xmlfile = "D:\\党建项目\\doc\\XML\\mxml-2.5e\\test.xml";
        if (!doc)
	{
...
int strncasecmp(char* str1, char* str2, int size)		
{
	int i,length;
	int flag = 0;
	char ch1, ch2;

	if(size>(int)strlen(str1)||size>(int)strlen(str2))
...
/*
 * End of "$Id: mxmldoc.c,v 1.1.1.1 2008/12/19 01:08:14 fyang Exp $".
 */
